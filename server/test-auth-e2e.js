...
async function runTests() {
  ...
  try {
    ...
    if (meRes.status !== 200) {
      throw new Error(
        `Authenticated /auth/me failed with status ${meRes.status}`,
      );
    }
    ...
    if (meData.user?.email !== "alex@nexai.test") {
      throw new Error(
        `Expected email alex@nexai.test, got ${meData.user?.email}`,
      );
    }
    console.log(
      `✓ Session successfully authenticated: ${meData.user.name} (${meData.user.email})`,
    );
    ...
    console.log(
      "✓ Bearer token header authentication succeeded (Extension compatible)",
    );
    ...
    if (
      !logoutCookie ||
      (!logoutCookie.includes("token=;") && !logoutCookie.includes("Max-Age=0"))
    ) {
